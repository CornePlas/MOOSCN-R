...
findSol <- function(d, s){
  ...
  while( (is.matrix(d)) && (length(d[,1] > 0)) ){ # create loop to end with one variable
    ...
    cat(format(Sys.time(), "%H:%M;"), " iteration: ",iterations,", size: ",nrow(d),"/",size,", statements:",(pref.s/2), file="output_small.txt", append=TRUE); 
...
for(j in 1:length(sample_size)){
  for (i in 1:length(no_statements)){
    if (sample_size[j] >= (no_statements[i]*2)){
      ...
      results[j,] <- c(sample_size[j], no_statements[i], sys[1], x[4], x[5], x[6]); 
...
print(results)
write.matrix(results, file="results_all.csv", sep=";", 1024);

cat('',file="output_large.txt");
sample_large <- c(75, 100, 125, 150, 175, 200);
statements <- 25;
results <- matrix(numeric(1), 6, 3);
for (i in 1:length(sample_large)){
  ...
  cat(results[i,],"\n");
  cat(results[i,],'\n', file="output_large.txt", sep=",", append=TRUE);
}
